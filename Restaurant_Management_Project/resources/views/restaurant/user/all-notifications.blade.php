...
<div class="container mt-3">
    ...
    @else
        ...
        <h1 class="mb-2 fw-bolder text-warning font">All Notifications</h1> 
        <div class="text-end"><a href="{{ route('clear.all.notifications') }}"  class="btn btn-outline-danger">Clear All</a></div>  
                ...
                <div class="card mb-3 border-0">
                    <div class="table-responsive mt-4">
                        ...
                        @foreach($allNotifications as $data)
                            <tr>
                                <td>
                                    <img src="Food_images/{{ $data->image }}" alt="">
                                </td> 

                                <td>{{ $data->food_name }}</td>   

                                @if ($data->food_type == 'fastfood')
                                    <td>Fast Food</td>
                                @elseif ($data->food_type == 'salads')
                                    <td>Salads</td>   
                                @else
                                    <td>Sushi</td>                              
                                @endif
                                ...
                                <td>${{ $data->price }}</td>
                                <td><a href="{{ url('/#menu') }}">check out</a></td>
                                ...
                                <td>                                        
                                    <button class="btn btn-danger delete-notification-button" data-notification-id="{{ $data->id }}">Clear</button>
                                </td>                                        
                            </tr>
        ...
        <div class="modal" id="confirmNotificationDeletionModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <span class="text-danger">Are you sure you want to Delete Notifications from your Notifications records?</span>

                        <div class="text-muted mt-4">
                            Once Notifications are Deleted, all of it's resources and data will be permanently delete. Please press on Delete button to confirm you would like to permanently Delete  Notifications from your Notifications records. 
                        </div>
                    </div>
...
<script>
    $(document).ready(function() {
        $('.delete-notification-button').on('click', function() {
            ...
            var modal = $('#confirmNotificationDeletionModal');
            modal.find('a.btn-outline-danger').attr('href', '/userNotificationDelete' + notificationId); ///// jokhon kew amar delete confirmation model ar moddhe jei delete button ache oi button aaa click korbe tokhon amader oi user ar id ta akhan theke pass hoye jabe amader route/web.php file ar moddhe /userNotificationDelete{id}
            modal.modal('show');
        });

        $('#cancelButton').on('click', function() {
            $('#confirmNotificationDeletionModal').modal('hide');
        });
    });
</script>
