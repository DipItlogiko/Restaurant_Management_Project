...
@section('body')
    <div class="p-5">
        ...
        @else
            ...
            <h1 class="mb-2 fw-bolder text-warning font">Workers Info</h1>   
                    ...
                    <div class="card mb-3 border-0">
                        <div class="table-responsive mt-4">
                            ...
                            @foreach($workers as $worker)
                                <tr>
                                    <td>
                                        <img src="Worker_images/{{ $worker->image }}" alt="">
                                    </td> 
                                    ...
                                    <td>
                                        ...
                                        <button class="btn btn-danger delete-worker-button" data-worker-id="{{ $worker->id }}">Delete</button>
                                    </td>                                        
                                </tr>
                            ...
                            <div class="row m-2 pt-3">
                                {{ $workers->appends(request()->query())->links('pagination::bootstrap-5')}}
                            </div>
            ...
            <div class="modal" id="confirmWorkerDeletionModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
...
<script>
    $(document).ready(function() {
        $('.delete-worker-button').on('click', function() {
            var workerId = $(this).data('worker-id');
            var modal = $('#confirmWorkerDeletionModal');
            modal.find('a.btn-outline-danger').attr('href', '/deleteWorker' + workerId); ///// jokhon kew amar delete confirmation model ar moddhe jei delete button ache oi button aaa click korbe tokhon amader oi user ar id ta akhan theke pass hoye jabe amader route/web.php file ar moddhe /deleteWorker{id}
            modal.modal('show');
        });

        $('#cancelButton').on('click', function() {
            $('#confirmWorkerDeletionModal').modal('hide');
        });
    });
</script>
